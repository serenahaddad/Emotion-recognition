...
class Net(nn.Module):
    ...
    def forward(self, x_img, x_aud):
        ...
        #print(x_aud.shape)
        x_aud = torch.flatten(x_aud, start_dim=1)
        #print(x_aud.shape)
...
def train(loader, net, criterion, device, optimizer):
    ...
    for epoch in range (30):
        ...
        audio_acc.append(100 *aud_correct/comb_total)
        image_acc.append(100 *img_correct/comb_total)   
        com_acc.append(100 *comb_correct/comb_total)   
    ...
    plt.plot(audio_acc, 'g', label='Audio Accuracy')
    plt.plot(image_acc, 'b', label='Image Accuracy')
    plt.plot(com_acc, 'r', label='Fusion Accuracy')
    plt.title('Validation Accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig('AccuracyFusionPlot.png')
    plt.show()
...
def plot_confusion_matrix(confusion_matrix, class_names):
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(normalized_matrix, cmap='Reds')
    ...
    for i in range(len(class_names)):
        for j in range(len(class_names)):
            text = ax.text(j, i, f'{normalized_matrix[i, j]:.2f}', ha="center", va="center", color="w")

    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.title('Confusion Matrix')
    plt.savefig('confusion_matrix_heatmap_Fusion.png')
    plt.show()
def test(loader, net, criterion, device, optimizer):
    ...
    np.save('confusion_matrix_fusion', confusion_matrix)
